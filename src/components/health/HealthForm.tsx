    ...
    <Card className="rounded-3xl border border-border/50 bg-card shadow-soft">
      <CardHeader className="space-y-3">
        <div className="flex items-center justify-between gap-4">
          <div>
            <CardTitle>Data ya Afya</CardTitle>
            <CardDescription>Ingiza vipimo vyako vya hivi karibuni.</CardDescription>
          </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField control={form.control} name="systolic"
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input type="number" {...field} min={70} max={250} className="h-12" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField control={form.control} name="diastolic"
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input type="number" {...field} min={40} max={150} className="h-12" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
            ...
            <div className="grid gap-4 grid-cols-2">
              <FormField control={form.control} name="heartRate"
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Mapigo (bpm)</FormLabel>
                    <FormControl><Input type="number" {...field} min={30} max={220} className="h-12" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField control={form.control} name="temperature"
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Joto (°C)</FormLabel>
                    <FormControl><Input type="number" {...field} min={34} max={42} step={0.1} className="h-12" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
            ...
            <FormField control={form.control} name="weight"
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Uzito (kg)</FormLabel>
                  <FormControl><Input type="number" {...field} min={20} max={300} step={0.1} className="h-12" /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <CardFooter className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-0">
              <p className="text-sm text-muted-foreground">BP: <span className="font-semibold">{bpStatus}</span></p>
