...
export default function Patients() {
  ...
  const filtered = patients.filter(p => {
    ...
    if (filter === "Recent Visit") return p.lastVisit >= "2026-04-07";
    return true;
  });
          ...
          <button key={p.id} onClick={() => navigate(`/patients/${p.id}`)} className="flex items-center gap-4 rounded-xl border bg-card p-4 hover:shadow-md transition-all text-left animate-fade-in">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className={`text-[10px] shrink-0 ${riskBadge[p.riskLevel]}`}>{p.riskLevel}</Badge>
              </div>
              <p className="text-xs text-muted-foreground mt-0.5">{p.age}y • {p.gender} • {p.condition}</p>
              <p className="text-xs text-muted-foreground mt-0.5">Last visit: {p.lastVisit} • BP: {p.bp}</p>
            </div>
          </button>
        ))}
      </div>
